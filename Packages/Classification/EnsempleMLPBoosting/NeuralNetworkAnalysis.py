...
class NeuralNetworkAnalysis(object):
    ...
    def train_test_split(self,test_size=0.2):
        ...
        for iclass, class_label in enumerate(self.class_labels):
            position = np.where(self.all_trgt==iclass)[0]
            data_classX = self.all_data[position]
            trgt_classX = self.all_trgt[position]
            X_train, X_test, y_train, y_test = train_test_split(data_classX, trgt_classX, test_size=test_size)
            list_data_train.append(X_train)
            list_data_test.append(X_test)
            list_trgt_train.append(y_train)
            list_trgt_test.append(y_test)

        Data_train = np.concatenate(list_data_train)
        Data_test = np.concatenate(list_data_test)
        trgt_train = np.concatenate(list_trgt_train)
        trgt_test = np.concatenate(list_trgt_test)
        return Data_train,Data_test,trgt_train,trgt_test
    ...
    def infoEachData(self,class_specific):
        ...
        return [class_specific,specific_data.shape[0]]
    
    def infoData(self):
        list_data = []
        for iclass in self.getClassLabels():
             list_data.append(self.infoEachData(iclass))
    ...
    def split_data(self):
        all_data,all_tgrt,_ = self.getData()
