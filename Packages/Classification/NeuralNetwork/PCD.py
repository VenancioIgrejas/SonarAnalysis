...
class PCDIndependent(object):
	...
	# Save configuration about size of dataset that will use in PCD
		...
		#
		#if os.path.exists(fileModelPCD):
            	#	print "loading PCDs models and train details"
		#	return self
		...
		for ipcd in range(self.n_components):
			...
			if ipcd == 0:
				...
				csv_logger = callbacks.CSVLogger(os.path.join(path,'{0}_numComponent_training.csv'.format(ipcd)))
				

				# reduce learning rate to train faster
				reduce_lrn = callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10,
