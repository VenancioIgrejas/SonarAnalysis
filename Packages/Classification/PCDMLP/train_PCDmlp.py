...
def train_pool(ifold):
    ...
    with timer('fit MLP PCD'):
        #hn.fit(X=all_data, y=all_trgt, train_id=train_id, test_id=test_id, ifold=args.ifold)
        pcd.fit(inputdata=all_data, targetdata=all_trgt)
    telegram_send.send(messages=["O seu treinamento MLP com PCD e os seguintes parametros acabou de terminar!\n"+
                                 "\nFOLD: "+str(ifold)+
