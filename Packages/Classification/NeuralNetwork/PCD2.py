...
class TrnParams(object):
	...
	n_epochs=1000,batch_size=512,init=10):
		self.init = init
				...
				if os.path.exists(fileModelPCD):
					...
					for i_init in range(trn_params.init):
						...
						sgd = SGD(lr=trn_params.learning_rate, decay=trn_params.learning_decay,
		                		...
		                		adam = Adam(lr=trn_params.learning_rate, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
	                
						# compile the model
						my_model.compile(loss='mean_squared_error', optimizer=sgd, metrics=['accuracy',ind_SP])
