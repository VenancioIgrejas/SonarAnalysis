...
parser.add_argument("--partition-data",default='train',type=str,help= "Select partition of data for extract PCD (train or test)")
...
if args.partition_data == "test":
    print "extract of componentes of PCD for teste data started (specialist {0})".format(args.specialist)
    ispec.set_partition_data('test')
    ispec.transform_data_pcd(data=X_test,trgt=y_test,fold=args.fold)
    print "extract of componentes of PCD for teste data finished (specialist {0})".format(args.specialist)
    sys.exit()
