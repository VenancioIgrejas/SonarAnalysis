...
if args.dev:
    results_path_specific = results_path + '/dev' + '/Hierarq_spec_' + trparamns.get_params_str() + '/'
else:
    results_path_specific = results_path + '/Hierarq_spec_' + trparamns.get_params_str() + '/'
...
from Functions.ensemble import HierarqNet,SpecialistClass
...
sp = SpecialistClass(mlp,dir='./',verbose=args.verbose)

hn = HierarqNet(sp,n_jobs=None,n_folds=args.fold,verbose=args.verbose,dir=results_path_specific)
...
for name, i in hn.estimators_.iteritems():
    ...
    index_ynew = np.array(range(y_new.shape[0]))
